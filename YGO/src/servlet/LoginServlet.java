...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (users.length != 0) 
			...
			if (m > 0) {// 成功
				out.println("<script>alert('删除成功！');window.location.href='Login/Delete-Login.jsp'</script>");
				out.close();
			} else {// 失败
				out.println("<script>alert('删除失败，请重试！');window.location.href='Login/Delete-Login.jsp'</script>");
				out.close();
			}
		}
	}
