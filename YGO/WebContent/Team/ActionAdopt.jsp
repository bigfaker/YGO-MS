	...
	ResultSet rs = null;
	   try{conn=DBHelper.getConnection();
	       ...
	       stmt = conn.prepareStatement(sq);
			...
			if(temp>0){
				out.println("<script language='javaScript'>alert('保存成功'); window.location.href='Team.jsp'; </script>");}
			else
			{
				out.println("<script language='javaScript'>alert('保存失败'); window.location.href='Team.jsp'; </script>");
			}
