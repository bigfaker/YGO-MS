...
<%
	...
	ResultSet rs = null;
	   try{conn=DBHelper.getConnection();
	       ...
	       String sq ="insert into Teaminfo(TeamName,TeamSign,CaptainName,CaptainQQ,Honor,HonorScore,Cleared,Opened)values("+str+");"; 
	       stmt = conn.prepareStatement(sq);
			int temp=stmt.executeUpdate();
			if(temp>0){
				out.println("<script language='javaScript'>alert('添加成功'); window.location.href='Team.jsp'; </script>");}
			else
			{
				out.println("<script language='javaScript'>alert('添加失败'); window.location.href='Team.jsp'; </script>");
			}
